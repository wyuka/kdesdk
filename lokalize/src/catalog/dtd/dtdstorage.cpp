...
DtdStorage::DtdStorage()
 : CatalogStorage()
{
    TranslatableQt::init();
    m_fileType = new DtdFileTypeQt();
...
int DtdStorage::capabilities() const
{
    return TwoFiles;
}
...
int DtdStorage::load(QIODevice* device)
{
...
int DtdStorage::loadSource(QIODevice* device)
{
    QTextStream in(device);
    QString inputContents = in.readAll();
...
bool DtdStorage::save(QIODevice* device, bool belongsToProject)
{
...
CatalogString DtdStorage::catalogString(int entry, DocPosition::Part part) const
{
    CatalogString cs;
...
CatalogString DtdStorage::catalogString(const DocPosition& pos) const
{
    return catalogString(pos.entry, pos.part);
}

CatalogString DtdStorage::targetWithTags(DocPosition pos) const
{
    return catalogString(pos.entry, DocPosition::Target);
}
CatalogString DtdStorage::sourceWithTags(DocPosition pos) const
{
    return catalogString(pos.entry, DocPosition::Source);
}

QString DtdStorage::source(const DocPosition& pos) const
{
    return catalogString(pos.entry, DocPosition::Source).string;
}
QString DtdStorage::target(const DocPosition& pos) const
{
    return catalogString(pos.entry, DocPosition::Target).string;
}

void DtdStorage::targetDelete(const DocPosition& pos, int count)
{
...
void DtdStorage::targetInsert(const DocPosition& pos, const QString& arg)
{
    QString newString = target(pos);
...
void DtdStorage::targetInsertTag(const DocPosition& pos, const InlineTag& tag)
{
    /*
...
InlineTag DtdStorage::targetDeleteTag(const DocPosition& pos)
{
    /*
...
void DtdStorage::setTarget(const DocPosition& pos, const QString& arg)
{
    m_translatable->setStringForEntryIndex(pos.entry, "t", arg);
}


QVector<AltTrans> DtdStorage::altTrans(const DocPosition& pos) const
{
...
QMap<QString,Tool> DtdStorage::allTools() const
{
...
QStringList DtdStorage::sourceFiles(const DocPosition& pos) const
{
    return QStringList();
}

QVector<Note> DtdStorage::notes(const DocPosition& pos) const
{
    Q_UNUSED(pos);
    //TODO
    return QVector<Note>();
...
QVector<Note> DtdStorage::developerNotes(const DocPosition& pos) const
{
    QList<Note> result;
...
Note DtdStorage::setNote(DocPosition pos, const Note& note)
{
    return note;
}

QStringList DtdStorage::context(const DocPosition& pos) const
{
    Q_UNUSED(pos);
    //TODO
...
QStringList DtdStorage::matchData(const DocPosition& pos) const
{
    Q_UNUSED(pos);
    return QStringList();
}

QString DtdStorage::id(const DocPosition& pos) const
{
    //return unitForPos(pos.entry).attribute("id");
    return QString(pos.entry);
}

bool DtdStorage::isPlural(const DocPosition& pos) const
{
    //return m_plurals.contains(pos.entry);
    return false;
}

bool DtdStorage::isEmpty(const DocPosition& pos) const
{
